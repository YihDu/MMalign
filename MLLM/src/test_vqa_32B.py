    ...
    with torch.inference_mode():
        for batch_idx, batch in enumerate(tqdm(loader, desc="Multilingual Batches")):
            ...
            try:
                ...
                # 打印前 50 个 token
                try:
                    print("First 50 input_ids: " + str(inputs["input_ids"][0][:50].tolist()))
                except Exception:
                    print("Failed to print input_ids preview.")
                ...
                try:
                    ...
                    print("vision token positions: " + str(pos[:20]))
                except Exception:
                    print("Failed to locate vision tokens.")

                # 打印对应文本（非常重要）
                try:
                    kill_text = batch['texts'][0].replace("\n", "\\n")[:300]
                    print("Sample text preview: " + kill_text)
                except Exception:
                    print("Failed to print sample text preview.")
